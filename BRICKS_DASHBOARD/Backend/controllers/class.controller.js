  ...
  try {
    ...
    if (!instructor.batches.includes(batch)) {
      return res
        ...
        .json({
          errors: [
            { msg: "You are not authorized to create classes for this batch" },
          ],
        });
    }
    ...
    if (new Date(scheduledAt) <= new Date()) {
      return res
        .status(400)
        .json({
          errors: [{ msg: "Scheduled date and time must be in the future" }],
        });
    }
    ...
    const newClass = await classModel.create({
      ...
      instructorName: `${instructor.fullname.firstname} ${
    ...
    res
      .status(500)
      .json({ errors: [{ msg: "Server error while creating class" }] });
  }
};
...
const getClassById = async (req, res) => {
  try {
    ...
    if (
      userType === "instructor" &&
      classData.instructor.toString() !== user._id.toString()
    ) {
      return res.status(403).json({
        errors: [{ msg: "You are not authorized to view this class" }],
      });
    ...
    res
      .status(500)
      .json({ errors: [{ msg: "Server error while fetching class" }] });
  }
};
...
const updateClass = async (req, res) => {
  try {
    ...
    if (
      ...
      new Date(updateData.scheduledAt) <= new Date()
    ) {
      return res
        .status(400)
        .json({
    ...
    res
      .status(500)
      .json({ errors: [{ msg: "Server error while updating class" }] });
  }
};
...
const deleteClass = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ errors: [{ msg: "Server error while deleting class" }] });
  }
};
...
const recordAttendance = async (req, res) => {
  try {
    ...
    if (classData.batch !== student.batch) {
      return res.status(403).json({
        errors: [
          { msg: "You are not authorized to record attendance for this class" },
        ],
      });
    }
    ...
    if (existingAttendance) {
      return res.status(400).json({
        errors: [
          { msg: "You have already recorded attendance for this class" },
        ],
      });
    }
    ...
    if (["completed", "cancelled"].includes(classData.status)) {
      return res
        ...
        .json({
          errors: [
            { msg: "Cannot record attendance for completed/cancelled class" },
          ],
        });
    }
    ...
    res
      .status(500)
      .json({ errors: [{ msg: "Server error while recording attendance" }] });
  }
};
...
const getClassStatistics = async (req, res) => {
  try {
    ...
    if (classData.instructor.toString() !== instructor._id.toString()) {
      return res.status(403).json({
        errors: [
          { msg: "You are not authorized to view this class statistics" },
        ],
      });
    }
    ...
    res
      .status(500)
      .json({
        errors: [{ msg: "Server error while fetching class statistics" }],
      });
  }
};
