...
const login = async (req, res) => {
  try {
    ...
    if (!student) {
      return res
        .status(400)
        .json({ errors: [{ msg: "Invalid enrollment number or password" }] });
    }
    ...
    if (student.mustChangePassword || student.isFirstLogin) {
      ...
      return res.status(200).json({
        ...
        tempToken,
        message:
          "Password change required. Please proceed to change your password.",
    ...
    res
      .status(200)
      .json({ token, student: studentResponse, message: "Login successful" });
  } catch (err) {
    console.error(err);
  ...
  try {
    ...
    if (student.mustChangePassword) {
      return res.status(403).json({
        mustChangePassword: true,
        msg: "Password change required. Please change your password to access your profile.",
      });
    }
    ...
    res
      .status(500)
      .json({ errors: [{ msg: "Server error while fetching profile" }] });
  }
};
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        decoded.enrollmentNumber !== enrollmentNumber
      ) {
        return res
          .status(400)
          .json({ errors: [{ msg: "Invalid token or enrollment number" }] });
      }
    } catch (err) {
      return res
        .status(400)
    ...
    const student = await studentModel
      .findOne({ enrollmentNumber })
      .select("+otpAttempts +otpBlockedUntil");
    ...
    if (student.otpBlockedUntil && Date.now() < student.otpBlockedUntil) {
      const minutesLeft = Math.ceil(
        (student.otpBlockedUntil - Date.now()) / 60000
      );
      return res.status(429).json({
        errors: [
          {
            msg: `Too many OTP requests. Please try again in ${minutesLeft} minutes.`,
          },
        ],
      });
    }
    ...
    if (!emailResult.success) {
      ...
      return res
        .status(statusCode)
        .json({ errors: [{ msg: emailResult.error }] });
    }
    ...
    res
      .status(500)
      .json({ errors: [{ msg: "Server error during OTP request" }] });
  }
};
...
const verifyOTPAndChangePassword = async (req, res) => {
  try {
    ...
    const student = await studentModel
      .findOne({ enrollmentNumber })
      .select(
        "+resetPasswordOtp +resetPasswordOtpExpiry +otpAttempts +otpBlockedUntil +password"
      );
    if (!student) {
    ...
    if (student.otpBlockedUntil && Date.now() < student.otpBlockedUntil) {
      ...
      return res.status(429).json({
        errors: [
          {
            msg: `Too many OTP attempts. Please try again in ${minutesLeft} minutes.`,
          },
        ],
      });
    }
    ...
    if (!student.verifyOTP(otp)) {
      ...
      if (student.otpAttempts >= 5) {
        ...
        return res.status(429).json({
          errors: [
            {
              msg: "Too many incorrect OTP attempts. You are blocked for 30 minutes.",
            },
          ],
        });
      }
    ...
    await otpService.sendPasswordChangeConfirmation(
      student.email,
      student.fullname.firstname
    );
    ...
    res
      .status(500)
      .json({ errors: [{ msg: "Server error during OTP verification" }] });
  }
};
...
const forgotPassword = async (req, res) => {
  try {
    ...
    const student = await studentModel
      .findOne({ enrollmentNumber, email })
      .select("+otpAttempts +otpBlockedUntil");
    if (!student) {
      return res.status(404).json({ errors: [{ msg: "Student not found" }] });
    }
    ...
    res
      .status(500)
      .json({
        errors: [{ msg: "Server error during forgot password process" }],
      });
  }
};
...
const resetPassword = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        errors: [{ msg: "Server error during reset password process" }],
      });
  }
};
