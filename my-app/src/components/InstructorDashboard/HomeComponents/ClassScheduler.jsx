...
import { FiClock, FiPlus, FiVideo, FiUsers, FiChevronLeft, FiChevronRight } from "react-icons/fi";
...
const ClassScheduler = () => {
  ...
  const getClassesForDay = (dayInfo) => {
    return classData.filter((cls) => {
      const clsDate = new Date(cls.scheduledAt);
      // Check if the class date matches the specific date, not just the day name
      return clsDate.toDateString() === dayInfo.fullDate.toDateString();
  ...
  const getTotalClassesForWeek = () => {
    // Count classes only in the displayed week
    return weekDates.reduce((total, dayInfo) => {
      return total + getClassesForDay(dayInfo).length;
    }, 0);
  };

  const getUpcomingClassesCount = () => {
    // Count upcoming classes only in the displayed week
    return weekDates.reduce((total, dayInfo) => {
      const dayClasses = getClassesForDay(dayInfo);
      return total + dayClasses.filter(cls => getClassStatus(cls.scheduledAt, cls.duration) === 'upcoming').length;
    }, 0);
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-center">
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-xs font-semibold text-orange-600">{getWeekLabel()}</span>
              {weekOffset !== 0 && (
                <button
